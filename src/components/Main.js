...
export default function Main(props) {
    ...
    return (
        ...
        <div className={styles.graph}>
            {[1,2,3,4,5].map(val => {
                return <div style={{top: `${val*20 - 20}%`}} 
                className={styles.gridLines} key={val}>
                    <p className={styles.values}>{((-val)*20*max/100).toFixed(0)
                    }</p>
                </div>
            })}
            {Object.keys(totals).map((val, index) => {
                return <div key={index} className={`${styles.bar} ${styles[`bar${index}`]}`} style={{ height: `${(totals[val] / max) * 100}%` }}>
